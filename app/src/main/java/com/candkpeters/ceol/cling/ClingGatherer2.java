...
public class ClingGatherer2 extends GathererBase implements Runnable {
    ...
    private ContentSourceSubscriptionManager contentSourceSubscriptionManager;
    ...
    private void bindToCling() {
        ...
        if ( !isClingServiceBound) {
            ...
            contentSourceSubscriptionManager = new ContentSourceSubscriptionManager(context, ceolModel, new OnSubscriptionListener() {
...
//            Log.d(TAG, "FriendlyName = " + friendlyName);
            ...
            if ( friendlyName.compareToIgnoreCase(prefOpenhomeNmae) == 0) {
                Log.d(TAG, "deviceAdded: Aha - found Openhome: " + device.getDisplayString());

                openHomeSubscriptionManager.addDevice(upnpService, device);
            }
            if ( friendlyName.compareToIgnoreCase(prefContentSourceName) == 0) {
                Log.d(TAG, "deviceAdded: Aha - found content source: " + device.getDisplayString());

                contentSourceSubscriptionManager.addDevice(upnpService, device);
            }
